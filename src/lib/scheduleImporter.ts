import { db } from "@/db";
import { scheduleMatches, scheduleSettings, categories, matches } from "@/db/schema";
...
export async function performScheduleImport() {
  ...
  if (settings.length === 0 || !settings[0].csvUrl) {
    throw new Error("No schedule CSV URL configured");
  }
  ...
  // 3. Clear existing schedule matches for this tournament
  ...
  // 5. Update last imported timestamp + auto-generate category config
  ...
  await db
    ...
    .set({ 
      ...
      categoryConfig: JSON.stringify(newConfig) 
    })
    .where(eq(scheduleSettings.id, settings[0].id));

  // 6. SYNC CLUB INFO TO BRACKET (MATCHES TABLE)
